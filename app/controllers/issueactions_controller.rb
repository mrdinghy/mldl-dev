class IssueactionsController < ApplicationController
  ...
  def create
    ...
    if params[:issueaction][:mediation_id].present?
      ...
      if params[:reopenmediation]

          @issueaction.update_attributes!(actionbody: params[:reopennote])
          @mediation.update_attributes(:mediate_end => nil, :mediation_held => false)
          @mediation.save!
   ...
   if @issueaction.agenda?
      ...
      if @issueaction.save
        if !params[:issueaction][:mediation_id].nil? and !params[:issueaction][:mediation_id].blank?
      ...
      @issueaction.update_attributes!(:actionbody => params[:cancelnote])
      ...
      @issueaction.update_attributes!(:actionbody => params[:refernote])
     ...
     d2 = Date.parse(params[:mediate_start])
      ...
      @issue.update_attributes!(:status => Status::RESOLVED, resolutiontype_id: params[:resolutiontype_id],
                              resolution_date: params[:resolution_date], resolution: params[:resolution])
    ...
    if @issueaction.save
      ...
      # need to save comment with Old resoltion statement and clear out current
         ...
         if @issueaction.save
            @issue.update_attributes!(resolution_date: nil, cancelled_at: nil, status: Status::ONGOING)
            sendnotice = 'Issue Resolved successfully.'
          end
